    ...
    public class RecordPickingModel : PageModel
    {
        ...
        public IActionResult OnPostSave()
        {
            ...
            try
            {
                ...
                {
                    if(PickerId.HasValue && PickerId.Value > 0)
                    {
                        _pickingservices.Picking_RecordPicking(OrderId.Value,PickerId.Value, PickedItems);
                        FeedBackMessage = "Order Picking has been recorder.";
                    }
                    else
                    {
                        throw new Exception("Missing or incorrect picker number");
                    }
                }
                else
                {
                    throw new Exception("Missing or incorrect Order number");
                }
